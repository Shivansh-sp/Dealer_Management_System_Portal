...
const matchCriteria = (item: any, filter: any): boolean => {
  ...
  for (const key of Object.keys(filter)) {
    ...
    if (key === '$or' && Array.isArray(val)) {
      return val.some(subFilter => matchCriteria(item, subFilter));
    }
    if (key === 'isDeleted' && item.isDeleted !== val) {
      return false;
    }
    if (item[key] !== undefined) {
      if (val instanceof RegExp) {
        if (!val.test(item[key].toString())) return false;
      } else if (typeof val === 'object' && val !== null && val.toString().startsWith('/')) {
        const regexStr = val.toString().slice(1, -1);
        const r = new RegExp(regexStr, 'i');
        if (!r.test(item[key].toString())) return false;
...
const activateOfflineMockDb = async () => {
  ...
  mockStore.set('PurchaseOrder', [
    {
      ...
      quantity: 5,
      supplier: 'SMG Manufacturing India',
  ...
  mongoose.Query.prototype.exec = async function (this: any) {
    ...
    const matched = list.filter(item => matchCriteria(item, filter));
    ...
    if (op === 'findOne' || op === 'findOneAndRemove' || op === 'findOneAndUpdate') {
      ...
      if (op === 'findOneAndUpdate') {
        ...
        if (update) {
          const updatedDoc = { ...docObj, ...update };
